<?php
//conectar
function conect($usuario,$pass,$database){return($c=mysql_connect('localhost',$usuario,$pass))?mysql_select_db($database,$c):false;}
...
function select($_,$tab,$wr,$order){
	if (mysql_num_rows($sql=MYSQL_QUERY("SELECT * FROM $tab".($wr?" WHERE $wr":"").($oder?" ORDER BY $order":"")))){
		$j=0;
...
#em ordem (servidor,usuario,senha e banco de dados);
	//conect("floresde_sarom","sabedoria","floresde_sarom");
?>